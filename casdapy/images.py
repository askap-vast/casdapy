...
class CasdaSbidImageQuery:
    ...
    def run(
        self, download_destination: Path, adql_query: str = None, poll_period: int = 30
    ):
        tap_results = self._query(adql_query)
        downloaded_files = self._download_images_async(
            tap_results, download_destination, poll_period
        )
        self.logger.info(
            "Downloaded %d files into %s", len(downloaded_files), download_destination
        )
